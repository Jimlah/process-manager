<div class="select-none mb-2">
    ...
    <div class="flex items-center justify-between group hover:bg-accent/50 transition-colors">
        <button
            wire:click="toggle"
            class="flex items-center gap-2 py-1.5 px-2 flex-1 text-left min-w-0"
        >
            <x-icon name="chevron-down" class="w-3 h-3 text-muted-foreground transition-transform {{ $expanded ? 'rotate-0' : '-rotate-90' }}" />
            <x-icon name="folder" class="w-3.5 h-3.5 text-muted-foreground" />
            <span class="font-bold text-xs truncate uppercase tracking-wider text-foreground">{{ $project->name }}</span>
            ...
            @foreach($project->commands as $command)
                ...
                >
                    ...
                    @if($command->status === 'running')
                        <x-icon name="play" class="w-3 h-3 text-success shrink-0" />
                    @else
                        <x-icon name="stop" class="w-3 h-3 text-muted shrink-0" />
                    @endif
